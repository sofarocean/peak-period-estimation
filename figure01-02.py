...
def get_spectra(peak_frequency, standard_deviation, kind='gaussian'):
    ...
    non_monotone = downsampled_spectrum.interpolate_frequency(interpolated_frequencies, method='spline',monotone_interpolation=False)

    return {'target': true_spectrum, 'downsampled': downsampled_spectrum,'interpolated':interpolated_spectrum,'non_monotone':non_monotone}
...
def plot_example(kind,peak_frequency,step,xlab=True,ylab=True,text=None, log=False):
    ...
    plot(gaussian['downsampled'], block=False, label='linear', freq_scale=peak_frequency,scale=scale, color='grey',linestyle='--')
...
plt.subplot( 2,2,2)
plot_example('jonswap', 0.1025, 0.5*frequency_step,xlab=False,ylab=False,text=r'(b) JONSWAP'+ '\n' + r'$f_{peak}=0.105$ Hz',log=log)
